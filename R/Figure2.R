...
ton <- 3;toff <- 3.5
D1 <- data.table(tz=tz,value=150*exp(-0.05*tz),qty="notifications",scenario="basecase")
D2 <- data.table(tz=tz,value=15*exp(-0.05*tz),qty="mortality",scenario="basecase")
D3 <- data.table(tz=tz,
                 value=150*fcase(tz<ton,exp(-0.05*tz),
                                 tz>toff,0.9*exp(-(0.05/1.5)*tz),
                                 default = exp(-0.05*tz*2)*1.3),
                                 qty="notifications",scenario="ACF")
D4 <- data.table(tz=tz,
...
F1 <- ggplot(DALL,aes(tz,value,col=quantity,lty=scenario))+
  geom_line()+
  annotate("rect", xmin = ton-sft, xmax = toff-sft,
           ymin = 0, ymax = top,
           alpha = .2)+
  ...
  xlab("Time") + ylab("Notifications or deaths")
F1
...
top2 <- 300
F2 <- ggplot(DALL2[qty=="mortality"],aes(tz,cumdf,col=qty))+
  geom_line()+
  annotate("rect", xmin = ton-sft, xmax = toff-sft,
           ymin = 0, ymax = top2,
           alpha = .2)+
  ...
  theme(axis.text=element_blank())+
  xlab("Time") + ylab("Cumulative deaths averted")
F2
...
F4 <- ggplot(D7,aes(x=xs,xend=xe,y=ys,yend=ye))+
...
## 7 zone using above
X <- 30
F3 <- ggplot(D7,aes(x=xs,xend=xe,y=ys,yend=ye))+
  geom_point(shape=1,col="blue")+
  geom_segment(col="blue")+
  geom_segment(data=DR,col="blue",lty=2)+
  geom_point(data=DR,shape=1,col="blue")+
  geom_segment(x = X, y = 41, xend = X, yend = 127,
...
## ======== combined figure
FC <- (F1+F2+F3+F4+plot_layout(ncol=2,byrow=FALSE))+plot_annotation(tag_levels = 'A')
FC

ggsave(here("plots/VOI_concept.png"),w=8,h=8)
