...
updateARGS <- function(args,numpatches=7,simlength=73){
  ...
  args$MM <- diag(rep(1,numpatches)) + matrix(0.1,nrow = numpatches, ncol = numpatches)
  ...
  args$mu_HIV_int <- args$mu_ART_int <- matrix(0,nrow = 3, ncol = simlength) #HIV/ART mortality
...
makeITZ <- function(pops0,screenrate,burnin=0){
  ...
  ## TZ[TZ<5] <- 5                  #cap below at least 5 months
  sum(TZ) #~60 months
...
setACF <- function(args,screenrate,itz){
  for(i in 1:length(args$popinit)){
    ...
    if(length(itz[[i]])>0){
      args$ACFhaz0[i,itz[[i]]] <- args$ACFhaz1[i,itz[[i]]] <- screenrate / args$popinit[i]
    }
  }
  args
}
...
getnewcolnames <- function(args){
  tmpmod <- BLASTtbmod:::stocm$new(pars = args,
                                   time = min(args$tt) +  1,
                                   n_particles = 1)
  ## print(str(tmpmod))
...
getIndexList <- function(args){
  ...
  cnmz <- getnewcolnames(args) #get updated column data: see also cnmz$dim$TB_deaths
  idz <- cnmz$index$TB_deaths #column ids of deaths
  zdz <- rep(1:args$patch_dims,3*3)                                       #zones x (HIVxage)
  ## zdz <- rep(1:nzones,3*3)                                       #zones x (HIVxage)
  wcz <- list()                                                  #zone key
...
## ## calculates means
## aggregator <- function(N,k,x){
##   span <- N/k
##   if(N %% k) stop("k must divide N!")
##   xx <- rep(1:k,each = span)
##   xx <- split(x,xx)
##   xx <- lapply(xx,mean)
##   unlist(xx)
## }
