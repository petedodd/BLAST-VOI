...
ageMids <- c(7.5, 32.5, 60) #get from get.parms
ariparm <- 5e-2 # high?
...
## testo <- extract.pops(test, 1:dim(test)[2], out_type = c("incidence"))
## summary(testo)
## out_type from unique(gsub("[[:punct:]]+|[[:digit:]]+", "", BLASTtbmod::get_cols))
## look:
testo <- extract.pops(test, 1:dim(test)[2], out_type = c("notes", "N", "D", "SC"))
testo[varname=="SC",varname:="D"] #merge SC and D
testo <- testo[age != "0-14", .(value = sum(value)), by = .(particle,step, varname, patch)] # aggr/adult
testo <- dcast(testo, particle+step + patch ~ varname, value.var = "value")
## ggplot(testo, aes(step, 1e5 * notes / N,group=particle)) +
##   geom_line() +
##   facet_wrap(~patch)
...
## LTBI graph
testo <- extract.pops(test, 1:dim(test)[2], out_type = c("N", "LL", "LR")) #latent
testo <- testo[age != "0-14", .(value = sum(value)), by = .(particle,step, varname, patch)] # aggr/adult
testo <- dcast(testo, particle+step + patch ~ varname, value.var = "value")
ggplot(testo, aes(step, 1e2 * (LR) / N,group=particle)) +
  geom_line() +
  facet_wrap(~patch) +
  expand_limits(y = c(0, NA))
...
## initial state looks OK
L <- testo[step == 1]
L[, prevmod := 1e5 * D / N]
L <- merge(L, data.table(prevdat = prev, patch = paste0("Patch ", 1:7)), by = "patch")
## L <- merge(L, PPP[, .(patch, prevdat = 1e5 * prev.mid)], by = "patch")
...
## check infection flux:
idx2 <- grep("cum_note_flux", BLASTtbmod::get_cols)
(nmz2 <- grep("cum_note_flux", BLASTtbmod::get_cols, value = TRUE)) # check
tmp <- t(test[idx2, 1, ]) # matrix over time for particle 1
matplot(tmp, type = "l")
## empirical infection flux
EIF2 <- matrix(tail(tmp, n = 1), ncol = args$patch_dims, nrow = args$patch_dims)
(nEIF2 <- EIF2 / max(EIF2))
FM / max(FM)
...
matplot(tmp, type = "l")
## empirical infection flux
EIF <- matrix(tail(tmp, n = 1), ncol = args$patch_dims, nrow = args$patch_dims)
(nEIF <- EIF / max(EIF))
FM / max(FM)
...
CF <- data.table(
  ...
  output.FOI.flux = c(nEIF),
  output.note.flux = c(nEIF2)
)
CF[, variable := gsub("cum_inf_flux", "", nmz)]
CF[, fromid := gsub("\\[", "", gsub(",.\\]", "", variable))]
...
## ================= parameter checks ==========
## taken from BLASTtbmod:

## parameter mapping CHECK
## NOTES -> model
## omega  :  1/dur      #tb cessation
## delta  :  dtct_rate dtct_rate[i,step] #detection rate
...
Pomega  <-  1/args$dur      #tb cessation
Pdelta  <-  args$cdr/(args$dur*(1-args$cdr))  #detection rate TODO
Pmu     <-  args$mu_noHIV_int[2,1] #mortality: TODO possible to make mean?
Psigma  <-  args$pLL #stabilisation
Palpha  <-  args$pDf #fast progression
Pepsilon<-  args$pDs #slow progn
Pgamma  <-  args$progress_rate-args$regress_rate #symptom progn
Prho    <-  args$pDr  #relapse rate
Ptau    <-  1/0.5    #1/treatment dur
...
Av[1] <- -Pgamma * ppC /((Pgamma-Psigma-Palpha)*(Pomega+Pdelta-Psigma-Palpha)) + Pgamma * ppB /((Pgamma-Pepsilon)*(Pomega+Pdelta-Pepsilon)) -  Pgamma * (ppB/(Pgamma-Pepsilon)-ppC/(Pgamma-Psigma-Palpha))  / (Pomega+Pdelta-Pgamma)- Prho * Ptau * (ppF*(1/(Pomega+Pdelta-Psigma-Palpha)-1/(Pomega+Pdelta-Prho))/((Prho-Psigma-Palpha)*(Ptau-Psigma-Palpha))+ppG*(1/(Pomega+Pdelta-Pepsilon)-1/(Pomega+Pdelta-Prho))/((Prho-Pepsilon)*(Ptau-Pepsilon)) + ppH*(1/(Pomega+Pdelta-Pgamma)-1/(Pomega+Pdelta-Prho))/((Prho-Pgamma)*(Ptau-Pgamma)) + ppJ*(-1/(Pomega+Pdelta-Prho))/((Prho-Pomega-Pdelta)*(Ptau-Pomega-Pdelta))+ ppK *(1/(Pomega+Pdelta-Ptau)-1/(Pomega+Pdelta-Prho))/(Prho-Ptau))
Av[2] <- Pgamma * ppC /((Pgamma-Psigma-Palpha)*(Pomega+Pdelta-Psigma-Palpha)) + Prho * Ptau * (ppF/(Pomega+Pdelta-Psigma-Palpha))/((Prho-Psigma-Palpha)*(Ptau-Psigma-Palpha))
Av[3] <- - Pgamma * ppB /((Pgamma-Pepsilon)*(Pomega+Pdelta-Pepsilon)) + Prho * Ptau *  ( ppG/(Pomega+Pdelta-Pepsilon))/((Prho-Pepsilon)*(Ptau-Pepsilon))
Av[4] <- Pgamma * (ppB/(Pgamma-Pepsilon)-ppC/(Pgamma-Psigma-Palpha))/ (Pomega+Pdelta-Pgamma) + Prho * Ptau * (ppH/(Pomega+Pdelta-Pgamma) )/ ((Prho-Pgamma)*(Ptau-Pgamma))
Av[5] <- -Prho * Ptau * ( ppF/((Prho-Psigma-Palpha)*(Ptau-Psigma-Palpha)) + ppG/ ((Prho-Pepsilon)*(Ptau-Pepsilon)) + ppH/((Prho-Pgamma)*(Ptau-Pgamma)) + ppJ/((Prho-Pomega-Pdelta)*(Ptau-Pomega-Pdelta)) + ppK/(Prho-Ptau) )/(Pomega+Pdelta-Prho)
Av[6] <- Prho * Ptau * ((ppK / (Pomega + Pdelta - Ptau)) / (Prho - Ptau))

Av
## 2 & 6 are <0

## 2: Pgamma-Psigma-Palpha
## 6: 3 negatives


## see Documents/comms/JenLewis/flux_approx

M1 <- matrix(
  c(-(Psigma+Palpha+Pmu),0,0,0,0,0, #E
    Psigma,-(Pepsilon+Pmu),0,0,0,0, #L
    Palpha,Pepsilon,-(Pgamma+Pmu),0,0,0, #SC
    0,0,Pgamma,-(Pomega+Pdelta+Pmu),0,Prho, #CD
    0,0,0,Pdelta,-(Ptau+Pmu),0, #T
    0,0,0,0,Ptau,-(Prho+Pmu)  #R
    ),
  nrow=6,ncol=6,byrow=TRUE
)
M1
...
## rerun blastP with above

## OK NOTE CHECK PASS SAME
cbind(c(A0,Av),c(Bfull,A1,A2,A3,A4,A5,A6))
